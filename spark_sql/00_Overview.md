# Spark SQL + Delta Lake — Visão Geral

Implementação do pipeline Northwind DW usando **Spark SQL** com **Delta Lake** como formato de armazenamento.

Os notebooks usam a API `spark.sql("""...""")` — SQL puro, sem DataFrames. O Delta Lake adiciona transações ACID, histórico de versões e operações de `MERGE`/`UPDATE` sobre arquivos Parquet.

## Pré-requisito

Para executar os notebooks, use o Docker do projeto (recomendado) ou configure o ambiente local. Veja o `README.md` na raiz do projeto.
...
| Notebook | O que faz |
|----------|-----------|
| `01_setup.ipynb` | Cria schemas e tabelas Delta Lake (bronze, silver, gold) |
...
| `15_delta_features.ipynb` | Recursos exclusivos Delta Lake: Time Travel, OPTIMIZE, Z-ORDER |

## Caminhos dos arquivos

Os notebooks apontam para `/workspace/pf_northwind/` — caminho padrão do Docker do projeto.

Para execução local fora do Docker, edite `BASE_DIR` em `utils.py`.
